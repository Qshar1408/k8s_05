...
1.1. Этот тип тома непосредственно подключает каталог с локального диска ноды (сервера) внутрь Pod'а.

1.2. Kubernetes не контролирует и не отвечает за фактическое содержимое этого каталога на хосте.

1.3. Операция удаления объектов PV и PVC аннулирует только их «виртуальные привязки» (объекты в кластере Kubernetes), но не затрагивает реальные файлы, оставшиеся на узловой машине.

2. Была применена политика Retain:

2.1. Даже для автоматически управляемых томов (например, облачных дисков AWS EBS или Google PD) политика Retain специально предотвращает стирание данных.

2.2. Это позволяет администратору вручную проверить, экспортировать или создать резервную копию данных перед окончательной очисткой.
